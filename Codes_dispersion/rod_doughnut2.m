...
NumberofPW=(2*n+1)^2;
disp('Forming regular Fourier matrix...'); 
...
%5X5
zz=[
-2 -2;-2 -1;-2 0;-2 1;-2 2; -1 -2;-1 -1;-1 0;-1 1;-1 2; 0 -2; 0 -1; 0 1; 0,2; 1 -2; 1 -1; 1 0; 1 1; 1 2;
...
disp('Modifying the defect matrix...');
R00=0.80*a;
f00=pi*R00^2/a^2;
...
for x=[-2:0.02:2].*a
    for y=[-2:0.02:2].*a
       fld0(ceil(x/0.02+101),ceil(y/0.02+101))=exp(i*(real(k+G)*x+imag(k+G)*y))*(hg0.*abs( k+G))';
        fld1(ceil(x/0.02+101),ceil(y/0.02+101))=exp(i*(real(k+G)*x+imag(k+G)*y))*(hg1.*abs( k+G))';
        fld2(ceil(x/0.02+101),ceil(y/0.02+101))=exp( i*(real(k+G)*x+imag( k+G)*y))*(hg2.*abs( k+G))';
        fld3(ceil(x/0.02+101),ceil(y/0.02+101))=exp(i*(real(k+G)*x+imag(k+G)*y))*(hg3.*abs(k+G))'; 
        fld4(ceil(x/0.02+101),ceil(y/0.02+101))=exp( i*(real( k+G)* x+imag( k+G)*y))*(hg4.*abs(k+G))';
        fld5(ceil(x/0.02+101),ceil(y/0.02+101))=exp(i*(real(k+G)*x+imag(k+G)*y))*(hg5.*abs( k+G))';
    end
end
fld0=fixphase(fld0);
fld1= fixphase(fld1);
...
axis image,
axis off,
title('R=0.8a,degenerated level 31,32'); 
subplot(2,2,2),surf(real(fld2+fld3)),
shading interp,view(2),
axis image,axis off,
title('R=0.8a,degenerated level 33,34'); 
subplot(2,2,3),surf(real(fld4)),
shading interp,view(2),
axis image,axis off,
title('R=0.8a,level 35');
subplot(2,2,4),
surf(real(fld5)),
shading interp,view(2),
axis image,axis off,
title('R=0.8a,level 36');


tmp=ifft2(ifftshift(eps20));
figure(3);imagesc(fftshift(real(tmp)))
%colormap(hot)
colormap(jet)
